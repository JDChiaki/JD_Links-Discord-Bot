			...
			if ep != 0:
				try:
					if links[ep] != 'No Data Yet':
						link_em = discord.Embed(title=f"{keys[anime]} Episode {args[1]}", url=links[ep], description=f"click to download :point_up: :point_up: \nIf the link is broken, DM {chiaki.mention} and ask for help~", color=0x252c8e)
					else:
				...
				ep0_em = discord.Embed(title=f"{keys[anime]} has no Episode 0!", color=0x000000)
				ep0_em.set_author(name=chiaki.display_name, icon_url=chiaki.avatar_url)
...
@client.command(pass_context = True , aliases=['PIC', 'P', 'p'])
async def pic(ctx, *args):
	...
	with open('pics.json', 'r', encoding='utf-8') as picsjson:
		...
		try:
			...
			pic_em.set_footer(text="photo requested by: {}".format(ctx.author.display_name), icon_url=ctx.author.avatar_url)
